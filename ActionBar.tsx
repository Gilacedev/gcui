					...
					{items.map((item, index) => (
						...
						>
							<a
								...
								onClick={item.icon === "fa fa-bell" ? (e) => { e.preventDefault(); handleToggleNotif(); } : undefined}
							>
								<span
									className={`${item.icon
							...
							{isNotifOpen && item.icon === "fa fa-bell" && (
								<div
									className={`absolute bottom-16 left-1/2 transform -translate-x-1/2 w-64 shadow-lg rounded-lg transition-transform duration-500 ease-out ${isNotifOpen
