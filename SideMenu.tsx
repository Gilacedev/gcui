			...
			<div className={"sticky w-[70%] bg-slate-900 z-10 max-h-[95vh] overflow-y-auto p-5 py-6 rounded-xl shadow-2xl text-slate-900 text-center "}>
				<div className={"flex flex-col"}>
					<div className="flex flex-col justify-center gap-2 items-center">
						...
						<Link href={"/auth"}>
							<span onClick={() => MenuStores.setMenu(false)} className="text-indigo-700 border-b  text-sm font-medium">
								{loading ? <Loader /> : isHydrated && isAuthenticated ? user?.name : Language().login}
							</span>
						</Link>
						<Link href={"/auth"}>
							<span onClick={() => MenuStores.setMenu(false)} className="text-slate-400  text-xs font-base ">
					...
					<div className="bg-slate-800 rounded-4xl p-5 grid grid-cols-2 mt-8 justify-center items-center gap-4">
						<Link href={isHydrated && isAuthenticated ? '' : "/auth"}>
							<div onClick={() => MenuStores.setMenu(false)} className="w-18 h-18 rounded-full flex justify-center items-center bg-slate-950 m-auto">
								<span className="far fa-user-cog  text-2xl text-indigo-700">
								</span>
							</div>
						</Link>
						<Link href={'/'}>
							<div onClick={() => MenuStores.setMenu(false)} className="w-18 h-18 rounded-full flex justify-center items-center bg-slate-200 m-auto">
					...
					<Link href={"/blog"}>
						<div onClick={() => MenuStores.setMenu(false)} className="text-slate-200 bg-slate-800 rounded-4xl p-3 flex mt-3 justify-center items-center gap-2">
